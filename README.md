...
| | |
|---|---|
| ![Dashboard — one pane of glass](screenshots/dashboard.png) **Dashboard** — live APIs, matches, alert queue, AI insights | ![Multi-Agent AI Council](screenshots/agent-council.png) **Agent Council** — 8 agents deliberate; conflicts resolved; one accountable decision (situation → prediction → reasoning → 92% confidence → actions → impact → rejected alternatives) |
